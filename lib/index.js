...
module.exports.filterCachedVersionList = function(event, context, callback) {
	console.time('BuildVersionListFromS3');
	buildList(event).then(function(data) {
		console.timeEnd('BuildVersionListFromS3');
//		console.log('Sending this data to Dynamo: ' + JSON.stringify(data));
...
//
module.exports.restoreFilteredCachedVersionList = function(event, context, callback) {
		console.time('PushToDynamo');
		pushToDynamo({ destinationTableName: event.DestinationTableName, filterCachedVersionList: event.filterCachedVersionList, bucket: event.Bucket }).then(function(returnedData) {
			console.timeEnd('PushToDynamo');
			console.log('Success!');
			console.log(JSON.stringify(returnedData));
			return callback(null, data);
