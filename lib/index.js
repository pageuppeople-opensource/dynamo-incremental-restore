...
function getVersion(restoreToDate) {
    ...
    function processVersion(date) {
        ...
        //Delete records that were created after point in time
        s3DataContents.forEach(function(data) {
            var diff = date - new Date(data.LastModified);
            if ((diff < 0) && (!(data.Key in newData))) {
                console.log('Deleteing ' + data.Key)
                newData[data.Key] = data;
                newData[data.Key].deletedMarker = true;
            }
        });
        return newData;
    }
