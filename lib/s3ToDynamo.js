...
function s3ToDynamoDb(params, versionList) {
    ...
    function completeKey(key) {
        ...
        if (completedKeys.length % 1000 === 0) {
    ...
    function _init() {
        ...
        if (params.filterCachedVersionList) {
            readLargeFile(params.filterCachedVersionList + '.restored')
                .then(function(data) {
                    completedKeys = data;
                })
            readLargeFile(params.filterCachedVersionList)
                .then(function(data) {
                    ...
                    getKeysForDestinationTable().then(function(keys) {
                        ...
                        async.parallelLimit(work, 5, function(err, results) {
                            bar1.stop();
                            if (err) {
                                console.log('Error');
                                console.log(err);
